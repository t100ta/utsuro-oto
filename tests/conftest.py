...
# Ensure the repo root is on sys.path so ``utsuro_oto`` is importable
...
def fresh_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    """Reset utsuro_oto.config to default state and redirect CONFIG_PATH to a temp dir.
    ...
    """
    import utsuro_oto.config as cfg
